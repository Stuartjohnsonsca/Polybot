...
function LegsPanel({ basket }: { basket: Basket }) {
  return (
    <section className="rounded-lg border border-border bg-panel">
      <div className="border-b border-border px-4 py-3 text-sm font-medium">Legs</div>
      <ul className="divide-y divide-border">
        {basket.legs.map((leg, idx) => (
          ...
          >
            ...
            <SideToggle
              value={leg.preferredSide}
              onChange={(side) => updateLeg(leg.marketId, { preferredSide: side })}
            />
            <button
              type="button"
